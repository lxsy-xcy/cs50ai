...
def load_data(filename):
    ...
    with open("shopping.csv") as file:
        ...
        month = {"Jan":0,"Feb":1,"Mar":2,"April":3,"May":4,"June":5,"Jul":6,"Aug":7,"Sep":8,"Oct":9,"Nov":10,"Dec":11}
        for row in reader:
            data = []
...
def evaluate(labels, predictions):
    ...
    for revenue in zip(labels,predictions):
        if revenue[0] == 1:
            positivenum+=1
            if revenue[1]==1:
                positivepre+=1
    ...
    sensitivity = 1.0*positivepre/positivenum
    specificity = 1.0*negativepre/negativenum

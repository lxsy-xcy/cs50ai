...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        new_knowledge = Sentence(set(),count)
        for i in range(9):
            if 0<=xx[i]+cell[0]<self.width and 0<=yy[i]+cell[1]<self.height:
                x = xx[i]+cell[0]
                y = yy[i]+cell[1]
                new_knowledge.cells.add((x,y))
    ...
    def make_safe_move(self):
        ...
        """
        choose = self.safes-self.moves_made
        print(f"choose:{choose}")
        if choose:
            return choice(list(choose))
